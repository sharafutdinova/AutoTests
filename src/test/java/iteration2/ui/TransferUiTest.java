...
public class TransferUiTest extends BaseUiTest {
  ...
  public void userCanTransferWithUsernameTest() {
    ...
    CreateAccountResponse senderAccountBefore =
        SessionStorage.getSteps().getCustomerAccount(createSenderAccountResponse.getId());
    ...
    new TransferPage()
        .performTransfer(
            createSenderAccountResponse.getAccountNumber(),
            username,
            createReceiverAccountResponse.getAccountNumber(),
            amount)
    ...
    CreateAccountResponse receiverAccount =
        SessionStorage.getSteps().getCustomerAccount(createReceiverAccountResponse.getId());
    softly.assertThat(receiverAccount.getBalance()).isEqualTo(amount);
    CreateAccountResponse senderAccountAfter =
        SessionStorage.getSteps().getCustomerAccount(createSenderAccountResponse.getId());
    double expectedSenderAmount =
        Math.round((senderAccountBefore.getBalance() - amount) * 100) / 100.0;
    softly.assertThat(senderAccountAfter.getBalance()).isEqualTo(expectedSenderAmount);
    Transaction lastReceiverTransaction =
        SessionStorage.getSteps().getAccountLastTransactions(createReceiverAccountResponse.getId());
    softly
        .assertThat(
            lastReceiverTransaction.validateTransaction(
                TransactionTypes.TRANSACTION_TYPE_FOR_TRANSFER_IN, amount))
        .isTrue();
    Transaction lastSenderTransaction =
        SessionStorage.getSteps().getAccountLastTransactions(createSenderAccountResponse.getId());
    softly
        .assertThat(
            lastSenderTransaction.validateTransaction(
                TransactionTypes.TRANSACTION_TYPE_FOR_TRANSFER_OUT, amount))
        .isTrue();
  }
  ...
  public void userCanNotTransferWithoutNameIfNameWasChangedTest() {
    ...
    new TransferPage()
        ...
        .performTransfer(
            ...
            amount)
        .checkAlertMessageAndAccept(
            BankAlert.RECIPIENT_NAME_DOES_NOT_MATCH_THE_REGISTERED_NAME.getMessage());
