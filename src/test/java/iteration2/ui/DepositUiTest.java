...
public class DepositUiTest extends BaseUiTest {
  ...
  public void userCanDepositTest() {
    ...
    new DepositPage()
        .performDeposit(createAccountResponse.getAccountNumber(), amount)
    ...
    GetAccountTransactionsResponse accountTransactions =
        SessionStorage.getSteps().getAccountTransactions(createAccountResponse.getId());
    softly.assertThat(accountTransactions.getTransactions().size()).isEqualTo(1);
    softly
        .assertThat(accountTransactions.getTransactions().getFirst().getAmount())
        .isEqualTo(amount);
    softly
        .assertThat(accountTransactions.getTransactions().getFirst().getType())
        .isEqualTo(TransactionTypes.TRANSACTION_TYPE_FOR_DEPOSIT.getDescription());

    CreateAccountResponse account =
        SessionStorage.getSteps().getCustomerAccount(createAccountResponse.getId());
  ...
  public void userCanNotDepositInvalidValueTest() {
    ...
    GetAccountTransactionsResponse accountTransactions =
        SessionStorage.getSteps().getAccountTransactions(createAccountResponse.getId());
    softly.assertThat(accountTransactions.getTransactions().size()).isZero();
    CreateAccountResponse account =
        SessionStorage.getSteps().getCustomerAccount(createAccountResponse.getId());
